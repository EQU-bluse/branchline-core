...
- `POST /scenarios/:id/events` appends an event (`type` required, `payload` optional object), increments the scenario `revision`, and returns `201` with the event. This works identically on branches: `sequence` continues after the copied prefix, and writes affect only that scenario. Later writes to a parent or sibling branch never alter an existing branch's history, `revision`, or metadata. An optional `occurredAt` pins the event time; it must be an exact UTC timestamp in the form `YYYY-MM-DDTHH:mm:ss.SSSZ`, denote a real date, and not be earlier than the scenario's previous event `occurredAt` (equal is allowed). When `occurredAt` is omitted, the event uses the scenario's virtual clock `currentTime` if a clock has been set (see below), otherwise the server generates the time; an explicit `occurredAt` keeps this validation and storage behavior regardless of any clock. Invalid or regressing values return a JSON `400` without appending an event or changing `revision`.
- `POST /scenarios/:id/clock` sets or advances the scenario's independent virtual clock. The request body must be a JSON object with a string `currentTime`: an exact UTC timestamp in the form `YYYY-MM-DDTHH:mm:ss.SSSZ` denoting a real date. The value must not be earlier than the scenario's existing clock `currentTime` or its last event's `occurredAt` (equal is allowed). Both the first set and later advances return `200` with `{ scenarioId, currentTime }`. A missing scenario returns the existing JSON `404`; an empty body, wrong media type, malformed or non-object JSON, a missing or non-string `currentTime`, an invalid date, or a value earlier than the current clock or last event return a JSON `400` with no side effects. Setting or reading the clock never changes the scenario `revision`, `events`, event `sequence` values, or issued pagination cursors.
- `GET /scenarios/:id/clock` returns `200` with `{ scenarioId, currentTime }`; `currentTime` is `null` when the clock has never been set. A missing scenario returns a JSON `404`. Reads have no side effects.
- A new branch does not inherit its parent's clock (it starts with `currentTime` `null`); clock operations and clock-defaulted events on a branch are isolated from the parent and sibling branches. A branch clock is still bounded by the copied prefix's last event `occurredAt`.
