...
- `GET /scenarios/:id/replay/explain?ruleId=:ruleId&sourceSequence=:n` explains a single replay result by re-deriving it from the scenario's current events and rules. Both query parameters are required, must appear exactly once, and must be non-empty; `sourceSequence` must be a positive decimal integer (no sign, decimal point, exponent, or whitespace), and any other query parameter returns a JSON `400`. It returns `200` with `{ scenarioId, revision, result, event, rule, ancestry }` only when `ruleId` belongs to this scenario, the scenario has an event at `sourceSequence`, and that rule's `when.type` matches the event's type (and therefore produces a result). `result` carries the full replay result fields, while `event` and `rule` are the full stored source event and rule. `ancestry` lists branch history from the current scenario back to the root, each entry `{ scenarioId, parentScenarioId, parentRevision }`; the root entry has both parent fields as `null`. A missing scenario returns the existing JSON `404`; an unknown or foreign `ruleId`, a missing `sourceSequence` event, a non-matching rule, and all invalid parameters return a JSON `400`. The query is read-only: success, failure, and repeated calls never change any scenario's revision, events, rules, clock, or existing pagination cursors.
