...
- `GET /scenarios/:id/replay/diff?against=:otherId` compares the current scenario's read-only replay results with another existing scenario's current replay results and returns `200` with `{ scenarioId, revision, againstScenarioId, againstRevision, added, removed }`. The first four fields identify the two scenarios and their current `revision`s. Both sides are computed with the same replay rules as `GET /scenarios/:id/replay`. Results are matched on `sourceSequence`, `type`, `payload`, and `occurredAt` only — `ruleId` is ignored; `payload` is compared by JSON structure and value, so objects whose fields are in a different order still compare equal. Results may repeat and duplicates offset one another one by one by occurrence count. `added` holds the results left only on the current scenario and `removed` those left only on the `against` scenario; each keeps its side's original replay order, and the elements carry the full replay result fields (including `ruleId`). The endpoint writes no data: queries, failed requests, and repeated queries never change either side's `revision`, events, rules, clock, or existing event pagination cursors. `against` must appear exactly once with a non-empty string value; a missing, empty, or repeated `against`, or any other query parameter, returns a JSON `400`. A missing scenario on either side returns the existing JSON `404` (the current scenario's `404` takes precedence over parameter validation).
